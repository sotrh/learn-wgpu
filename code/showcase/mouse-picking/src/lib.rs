...
struct State {
    ...
    mouse_ray: Ray,
    mouse_matrix: cgmath::Matrix4<f32>,
...
impl State {
    async fn new(window: Arc<Window>) -> anyhow::Result<State> {
        ...
        let mouse_pos = cgmath::vec2(0.0, 0.0);
        let mouse_ray = Ray::new(cgmath::vec3(0.0, 0.0, 1.0), cgmath::vec3(0.0, 0.0, 0.0));
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                self.mouse_pos = cgmath::vec2(
                    position.x as f32 / self.config.width as f32,
                    position.y as f32 / self.config.height as f32,
                );
                true
            }
            _ => false,
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        self.mouse_matrix = cgmath::Matrix4::from(self.camera_uniform.view_proj);

        let mouse_screen_pos = cgmath::vec3(self.mouse_pos.x, self.mouse_pos.y, 1.0);
        let mouse_world_pos = self
            .mouse_matrix
            .inverse_transform_vector(mouse_screen_pos)
            .unwrap();
